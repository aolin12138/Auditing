...
doc.add_paragraph(
    'On iris and wine, two continuous datasets with well-separated classes, adversarial-point '
    'density decreased as label noise increased from 0.1 to 0.5. On Car Evaluation, a '
    'categorical dataset with highly overlapping feature distributions, density increased. '
    'To isolate this effect, the experiment was repeated on three synthetic 3D datasets '
    'varying only in class separability (Figure 1). The well-separated dataset showed '
    'decreasing density; the overlapping dataset showed a slight increase; the categorical '
    'dataset produced a strong density increase. The direction of density change therefore '
    'depends on the separability of the original class distributions, not on the defect '
    'severity alone. The mechanism: when classes are well-separated, label noise causes the '
    'tree to create additional leaves to memorise corrupted labels, fragmenting the decision '
    'boundary and scattering adversarial points. When classes already overlap, the tree is '
    'near its representational ceiling; additional noise does not create many new boundary '
    'regions, and adversarial points remain clustered in the same confusion zones.'
)

fig('p2_label_noise.png',
    'Figure 1. Label noise: geometric spread and test accuracy for Tree + DTA (left) and '
    'SVM + HSJ (right). Solid lines show mean pairwise distance; dashed lines show test '
    'accuracy. Both metrics move together, providing no independent diagnostic signal.')

doc.add_paragraph(
    'Under the black-box HopSkipJump attack (Figure 1), the decision tree showed density '
    'decreasing (Cohen\'s d = \u22120.80 between noise levels 0.1 and 0.5), but test '
    'accuracy collapsed from 0.93 to 0.64. The SVM showed no clear density trend '
    '(d = \u22120.09), while accuracy also declined from 0.96 to 0.78. In all cases, '
    'the geometric signal either tracks accuracy or is null. Label noise provides no '
    'diagnostic information beyond what test accuracy already conveys.'
)
...
doc.add_paragraph(
    ...
    'geometric change depends on class separability: well-separated data shows decreasing '
    'density (more boundaries cause points to scatter), while overlapping or categorical '
    'data shows increasing density (few new boundary regions are created). This data-'
